...
public class Controller {
    ...
    private ArrayList<Order> sortOrderList(OrderList input) {
        ArrayList<Order> list = input.getOrders();

        for (int i = 0; i < list.size(); i++) {
            for (int j = 1; j < (list.size() - i); j++) {
                if (compareOrders(list.get(j - 1), list.get(j))) {
    ...
    private boolean compareOrders(Order orderA, Order orderB) {
        int hourA = orderA.getHasPickupTime() ? orderA.getTakeawayHour() : orderA.getHour();
        int hourB = orderB.getHasPickupTime() ? orderB.getTakeawayHour() : orderB.getHour();
